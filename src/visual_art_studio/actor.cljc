(ns visual-art-studio.actor
  "ArtOpsActor — the ISCO-08 2651 independent visual artist / manga studio
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one studio operation request (intake →
  ...
  The unconditional invariant: the ArtOpsAdvisor can never directly commit a
  record the VisualArtGovernor refuses — every commit-record! call is gated
  behind `:decide`, and page-delivery commits carry page/node counts derived
  from the kami-genko document model so the record describes exactly what
  was delivered."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [visual-art-studio.advisor :as advisor]
            [visual-art-studio.governor :as governor]
            [visual-art-studio.store :as store]))

(defn- doc-summary [doc]
  {:pages (count (:pages doc))
   :nodes (reduce + 0 (map (comp count :nodes) (:pages doc)))})

(defn build-graph
  "Build a compiled ArtOpsActor graph. `store` implements
  `visual-art-studio.store/Store`. `advisor` implements
  `visual-art-studio.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                  (fn [{:keys [request proposal]}]
                    (let [record (cond-> {:work-id (:work-id request)
                                          :op (:op proposal)
                                          :payload proposal}
                                   (= :deliver-page (:op proposal))
                                   (assoc :delivered (doc-summary (:doc request))))]
